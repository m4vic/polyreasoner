...
# Check for vision files starting with exp2_
files = sorted(glob.glob(os.path.join(results_dir, "results", "vision", "exp2_*.json")))
...
for f in files:
    try:
        with open(f, 'r') as file:
            ...
            run_tag = os.path.basename(f).split('_')[-2] # exp2_rev_cod_vision_run1_timestamp.json
            if "run" not in run_tag:
                 # try another split
...
for p, accs in sorted(pairs_data.items(), key=lambda x: sum(x[1])/len(x[1]), reverse=True):
    ...
    print(f"{p:<45} : {avg:.3f} (over {len(accs)} runs)")
